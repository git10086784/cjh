...
- (void)generateView {
    ...
    if (_buttonText!= nil && _buttonText != NULL) {
        _actionButton = [[UIButton alloc] initWithFrame:CGRectMake((CGRectGetMaxX(self.view.frame) / 2) - (contentWidth / 2), CGRectGetMaxY(self.view.frame) - kMainPageControlHeight - kActionButtonHeight - self.bottomPadding+35, contentWidth, kActionButtonHeight-10)];
        _actionButton.titleLabel.font = [UIFont fontWithName:self.buttonFontName size:self.buttonFontSize];
        [_actionButton setTitle:_buttonText forState:0];
        [_actionButton setTitle:_buttonText forState:1];
        [_actionButton setBackgroundColor:SYS_MAIN_COLOR_NORMAL];
